## Hi, I'm Bhumika 😊

🔎 Exploring patterns | 📈 Explaining insights | 🎨 Designing how data is understood  

## 👩‍💻 About Me
Working with data to uncover insights and build intuitive dashboards,
combining analysis with user-focused design.
---

## 🛠️ Skills
**Languages & Tools:**
![SQL](https://img.shields.io/badge/SQL-blue?style=flat-square)
![Python](https://img.shields.io/badge/Python-blue?style=flat-square)
![Excel](https://img.shields.io/badge/Excel-green?style=flat-square)
![Power BI](https://img.shields.io/badge/PowerBI-yellow?style=flat-square)

**Other Technologies:**
![.NET](https://img.shields.io/badge/.NET-512BD4?style=for-the-badge&logo=dotnet&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=nodedotjs&logoColor=white)
![Firebase](https://img.shields.io/badge/Firebase-FFCA28?style=for-the-badge&logo=firebase&logoColor=black)


**Design & Visualization:**
- Canva  
- Data Visualization Principles  
---

## 📁 Featured Projects
- **Mobile Usage Behavior Analysis**  
  Built interactive dashboards using Power BI to analyze user behavior  

- **Airline Passenger Satisfaction Analysis**  
  Performed data cleaning and EDA using Python  

- **Weekly Planner System**  
  Developed a database-driven system using SQL & .NET  

---

🎯 What I’m Working On
- Improving dashboard design skills  
- Exploring advanced data visualization  
- Building more real-world data analysis projects  

---


## 🔗 Connect with me
[https://www.linkedin.com/in/bhumika-kumaran-825919321](https://www.linkedin.com/in/bhumika-kumaran-825919321?utm_source=share_via&utm_content=profile&utm_medium=member_android)

⭐ *Exploring data with curiosity, shaping insights with creativity.*
